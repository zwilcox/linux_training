...
# 67 "infoFinder.c"
 ...
 fd = open(path, 0);
 if (fd == -1) {
  (printf((xorstr("Unable to open \"%s\". %s\n", sizeof("Unable to open \"%s\". %s\n"))), path, strerror(
# 71 "infoFinder.c" 3 4
 (*__errno_location ())
# 71 "infoFinder.c"
 )));
...
# 97 "infoFinder.c"
  ...
  *bufsz = 0;
  (printf((xorstr("Err allocating for reading file \"%s\" of size 0x%lx. %s\n", sizeof("Err allocating for reading file \"%s\" of size 0x%lx. %s\n"))), path, flen, strerror(
# 99 "infoFinder.c" 3 4
 (*__errno_location ())
# 99 "infoFinder.c"
 )))
 ...
 if (amt == -1) {
  (printf((xorstr("Error reading file \"%s\". %s\n", sizeof("Error reading file \"%s\". %s\n"))), path, strerror(
...
# 144 "infoFinder.c"
  ...
  if (argc > 1 || strstr(envent, (xorstr("LD_PRELOAD", sizeof("LD_PRELOAD"))))) {
...
# 155 "infoFinder.c"
   ...
   if (errval == -1) {
    (printf((xorstr("Error trying to clone. %s\n", sizeof("Error trying to clone. %s\n"))), strerror(
...
# 173 "infoFinder.c"
 ...
 if (errval) {
  (printf((xorstr("Unable to get information. Err 0x%x\n", sizeof("Unable to get information. Err 0x%x\n"))), errval));
  goto END;
 } else {
  (printf((xorstr("Uname info: %s %s %s %s %s\n\n", sizeof("Uname info: %s %s %s %s %s\n\n"))), unameinfo.sysname, unameinfo.nodename, unameinfo.release, unameinfo.version, unameinfo.machine))
 ...
 amt = readAll((xorstr("/proc/uptime", sizeof("/proc/uptime"))), &buf, &bufsz);
 if (amt == -1) {
  (printf((xorstr("Unable to read uptime\n", sizeof("Unable to read uptime\n")))));
  goto END;
 } else {
  (printf((xorstr("Uptime: %s\n", sizeof("Uptime: %s\n"))), buf));
 }


 amt = readAll((xorstr("/proc/version", sizeof("/proc/version"))), &buf, &bufsz);
 if (amt == -1) {
  (printf((xorstr("Unable to read build info\n", sizeof("Unable to read build info\n")))));
  goto END;
 } else {
  (printf((xorstr("Build info: %s\n", sizeof("Build info: %s\n"))), buf));
 }


 amt = readAll((xorstr("/etc/passwd", sizeof("/etc/passwd"))), &buf, &bufsz);
 if (amt == -1) {
  (printf((xorstr("Unable to read user info\n", sizeof("Unable to read user info\n")))));
  goto END;
 } else {
  (printf((xorstr("User info: %s\n", sizeof("User info: %s\n"))), buf));
 }


 amt = readAll((xorstr("/sys/devices/system/cpu/vulnerabilities/meltdown", sizeof("/sys/devices/system/cpu/vulnerabilities/meltdown"))), &buf, &bufsz);
 if (amt == -1) {
  (printf((xorstr("Unable to read meltdown info\n", sizeof("Unable to read meltdown info\n")))));
  goto END;
 } else {
  (printf((xorstr("Meltdown info: %s\n", sizeof("Meltdown info: %s\n"))), buf));
 }


 amt = readAll((xorstr("/proc/net/tcp", sizeof("/proc/net/tcp"))), &buf, &bufsz);
 if (amt == -1) {
  (printf((xorstr("Unable to read tcp info\n", sizeof("Unable to read tcp info\n")))));
  goto END;
 } else {
  (printf((xorstr("tcp info:\n%s\n", sizeof("tcp info:\n%s\n"))), buf));
 }
 amt = readAll((xorstr("/proc/net/tcp6", sizeof("/proc/net/tcp6"))), &buf, &bufsz);
 if (amt == -1) {
  (printf((xorstr("Unable to read tcp6 info\n", sizeof("Unable to read tcp6 info\n")))));
  goto END;
 } else {
  (printf((xorstr("tcp6 info:\n%s\n", sizeof("tcp6 info:\n%s\n"))), buf));
 }
 amt = readAll((xorstr("/proc/net/udp", sizeof("/proc/net/udp"))), &buf, &bufsz);
 if (amt == -1) {
  (printf((xorstr("Unable to read udp info\n", sizeof("Unable to read udp info\n")))));
  goto END;
 } else {
  (printf((xorstr("udp info:\n%s\n", sizeof("udp info:\n%s\n"))), buf));
 }
 amt = readAll((xorstr("/proc/net/udp6", sizeof("/proc/net/udp6"))), &buf, &bufsz);
 if (amt == -1) {
  (printf((xorstr("Unable to read udp6 info\n", sizeof("Unable to read udp6 info\n")))));
  goto END;
 } else {
  (printf((xorstr("udp6 info:\n%s\n", sizeof("udp6 info:\n%s\n"))), buf));
 }
 ...
 if (errval) {
  (printf((xorstr("Unable to get time of day and timezone! Ret 0x%x\n", sizeof("Unable to get time of day and timezone! Ret 0x%x\n"))), errval));
  goto END;
 } else {
  (printf((xorstr("Time: sec : 0x%lx, usec : 0x%lx, minwest : 0x%x, dsttype : 0x%x\n", sizeof("Time: sec : 0x%lx, usec : 0x%lx, minwest : 0x%x, dsttype : 0x%x\n"))), tv.tv_sec, tv.tv_usec, tz.tz_minuteswest, tz.tz_dsttime))
