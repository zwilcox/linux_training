...
static int __init mod_init(void) {
	...
	idt = (gate_desc*)idt_desc.address;
	div_zero_gate = &idt[DIVIDE_BY_ZERO_INDEX];
	orig_gate = *div_zero_gate;

	/* copy the gate and change values as desired */
	my_gate = *div_zero_gate;
	//my_gate.bits.type = 0xF;
	//my_gate.bits.dpl = 0x3;

	printk(KERN_ALERT "IDT is at %#016llx\n", (u64)idt);
	handler_addr = 
		((u64)div_zero_gate->offset_high) << 32 |
		((u64)div_zero_gate->offset_middle) << 16 |
		((u64)div_zero_gate->offset_low);
	 ...
	 * these. */
	error_entry_addr = get_abs_addr(handler_addr, 10);
	printk(KERN_ALERT "error_entry at %#016llx\n", error_entry_addr);

	error_exit_addr = get_abs_addr(handler_addr, 25);
	printk(KERN_ALERT "error_exit_addr is %#016llx\n", error_exit_addr);

	do_handler_addr = get_abs_addr(handler_addr, 20);
	...
	my_gate.offset_high = (u64)my_handler >> 32;
	my_gate.offset_middle = (u64)my_handler >> 16;
	my_gate.offset_low = (u64)my_handler;

	printk(KERN_ALERT "New gate's handler is %#016llx\n",
		((u64)my_gate.offset_high) << 32 | ((u64)my_gate.offset_middle) << 16 | ((u64)my_gate.offset_low));
	printk(KERN_ALERT "my_handler is %px\n", my_handler);

	/* Copy our new gate entry over our old one */
	memcpy(div_zero_gate, &my_gate, sizeof(gate_desc));
...
static void __exit mod_exit(void) {
	memcpy(div_zero_gate, &orig_gate, sizeof(gate_desc));
